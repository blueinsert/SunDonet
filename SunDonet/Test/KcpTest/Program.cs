...
namespace KcpTest
{
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            var sendContent = new ArraySegment<byte>(new byte[] { 0x01, 0x02 });
            // send client to server
            client.Send(sendContent, KcpChannel.Reliable);
            UpdateSeveralTimes(10);

            sendContent = new ArraySegment<byte>(new byte[] { 0x03, 0x04 });
            // send server to client
            int firstConnectionId = server.connections.Keys.First();
            server.Send(firstConnectionId, sendContent, KcpChannel.Reliable);
            UpdateSeveralTimes(10);
        }
    }
}
